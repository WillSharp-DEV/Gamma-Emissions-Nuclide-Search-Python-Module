...
GlobalLibrary = [
 ...
 {
   "E(keV)": "305.4( 2)",
   "Intensity": 0.00002655,
   "Nuclide": "Pu-237 (A 45.2 D)"
 },
 {
   "E(keV)": "305.5",
   "Intensity": 0.00000108,
   "Nuclide": "Dy-159 (EC 144.4 D)"
 },
 {
   "E(keV)": "306.1( 2)",
...
def StringHandler(Input: str) -> float:
    try:
        return float(re.sub(r'\(.*\)', '', Input))
    except:
        print(f"Could not convert '{Input}': StringHandle Operational Error.")
        return float(0)
...
class SearchConstructor:
    ...
    def InRange(self, Energy: float, LeftRightRange: float) -> List[ReturnItem]:
        ...
        for i in GlobalLibrary:
            ConvertedEnergy = StringHandler(i['E(keV)'])
            if ConvertedEnergy >= LeftVal and ConvertedEnergy <= RightVal:
                Item = ReturnItem(i['Nuclide'], i['Intensity'], ConvertedEnergy)
